    ...
    case 'info':
        {
            ...
            `.replace(/    /g, '').replace(/\t/g, '    '));
                if (outPath)
                    fs.rmSync(outPath, { maxRetries: 2, retryDelay: 500, recursive: true });
            }).catch(err => {
                console.error(err);
            });
        }
    ...
    case 'i':
        {
            ...
            autoLoad(args[0]).then(async ({ manifest, outPath }) => {
                ...
                await Promise.all(manifest.files.map(async (file) => {
                    ...
                    const url = JSON.parse(resTxt).data;
                ...
                switch (modloadertype) {
                    ...
                    `.replace(/    /g, '').replace(/\t/g, '    '));
                            ...
                            if (fs.existsSync('forge_installer.jar.log'))
                                fs.rmSync('forge_installer.jar.log');
